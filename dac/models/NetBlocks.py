...
def create_masks(input_size, hidden_size, n_hidden, input_order='sequential', input_degrees=None):
    ...
    if input_order == 'sequential':
        ...
        degrees += [torch.arange(input_size) % input_size -
                    1] if input_degrees is None else [input_degrees % input_size - 1]
...
class MaskedLinear(nn.Linear):
    ...
    def __init__(self, input_size, n_outputs, mask, cond_label_size=None):
        ...
        if cond_label_size is not None:
            self.cond_weight = nn.Parameter(torch.rand(
                n_outputs, cond_label_size) / math.sqrt(cond_label_size))
...
class MADE(nn.Module):
    def __init__(self, input_size, hidden_size, n_hidden, cond_label_size=None, activation='relu', input_order='sequential', input_degrees=None):
        ...
        self.net += [activation_fn,
                     MaskedLinear(hidden_size, 2 * input_size, masks[-1].repeat(2, 1))]  # model output out
    ...
    def forward(self, x, y=None):
        ...
        m, loga = self.net(self.net_input(x, y)).chunk(
            chunks=2, dim=1)  # out.chunk(2,1)
...
class MLP(nn.Module):
    ...
    def __init__(self, inp_dim, out_dim, hidden_layers=None, batchnorm=True, bias=True, out_relu=False):
        super().__init__()
