...
"""
@File: LASModule.py
@Time: 2023/09/11 17:51:05
@Author: Chaoqun Guo <chaoqunguo317@gmail.com>
@Version: 0.0.1
@Desc: label aware smoothing.
"""
...
class LabelAwareSmoothing(nn.Module):
    """Adjust label aware smoothing.
    @Time: 2023/09/11 19:40:24
    @Version : 0.0.1
    Attributes :
        Attributes: desc
    """

    def __init__(self, cls_num_list, smooth_head, smooth_tail, shape='concave', power=None, *args, **kwargs):
        super().__init__()
        ...
        if shape == 'concave':
            ...
            self.smooth = smooth_tail + \
                (smooth_head - smooth_tail) * \
                (np.array(cls_num_list) - n_K) / (n_1 - n_K)
