...
from dac.models.NetBlocks import MAF
...
class FlowGMM(nn.Module):
    def __init__(self, args):
        ...
        self.register_buffer("means", torch.randn(
            1, n_components, feat_dim)*center_std)  # (1, K, dim)
    ...
    def forward(self, x, cluster_mask: Optional[torch.BoolTensor] = None, return_lh_u=False):
        ...
        log_square = - 0.5 * \
            (u.unsqueeze(1) - self.means).square()
        log_p_u = self.neg_half_d_log_2_pi + log_square.sum(-1)
        log_p_x = log_p_u + \
            sum_log_abs_det.sum(-1, keepdim=True).expand(-1, self.n_components)
        ...
        log_lh = torch.logsumexp(masked_log_p_x, -1) - \
            log(float(n_component))  # (bz,)
        log_lh_u = torch.logsumexp(
            masked_log_p_u, -1) - log(float(n_component))  # (bz,)
